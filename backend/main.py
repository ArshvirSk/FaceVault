from fastapi import FastAPI, UploadFile, File, HTTPException, Depends, Cookie, Form
...
sessions = {}  # session_token -> {"user_id": int, "expires": datetime}
...
PROFILE_PHOTOS_DIR = os.path.join(os.path.dirname(__file__), "..", "data", "profile_photos")
...
async def register(
    ...
    temp_path = os.path.join(tempfile.gettempdir(), f"profile_{secrets.token_hex(8)}.jpg")
    ...
    try:
        ...
        if len(faces) > 1:
            raise HTTPException(status_code=400, detail="Multiple faces detected. Please upload a photo with only your face")
        ...
        # Create user
        user_id = db.create_user(username, password, profile_path, face_embedding)
...
async def scan_folder(request: ScanRequest, user_id: int = Depends(get_current_user)):
    ...
    async def generate_progress():
        try:
            ...
            album_name = request.album_name or os.path.basename(request.folder_path)
            album_id = db.add_album(album_name, request.folder_path, admin_user_id=user_id)
            ...
            for idx, image_path in enumerate(image_files):
                ...
                photo_id = db.add_photo(image_path, file_hash, album_id)
            ...
            if len(embeddings) > 0:
                ...
                for label in unique_labels:
                    if label == -1:  # Noise points - each gets own cluster
                        continue
                    person_id = db.add_person(cluster_id=int(label), album_id=album_id)
                    person_map[label] = person_id
                
                # Handle noise points - each gets its own person
                noise_cluster_id = max(unique_labels) + 1 if unique_labels else 0
                for face_id, label in zip(face_ids, labels):
                    if label == -1:
                        person_id = db.add_person(cluster_id=int(noise_cluster_id), album_id=album_id)
...
def rename_album(album_id: int, request: RenameRequest, user_id: int = Depends(get_current_user)):
    ...
    conn = sqlite3.connect(db.db_path)
    cursor = conn.cursor()
    cursor.execute("UPDATE albums SET name = ? WHERE album_id = ?", (request.name, album_id))
...
def add_album_member(album_id: int, request: AddMemberRequest, user_id: int = Depends(get_current_user)):
    ...
    # TODO: Trigger automatic face recognition for the new user
    
    return {"status": "success", "message": f"User {target_user['username']} added to album"}
...
def leave_album(album_id: int, user_id: int = Depends(get_current_user)):
    ...
    if not db.is_album_member(album_id, user_id):
        raise HTTPException(status_code=403, detail="You are not a member of this album")
    
    # Check if user is admin
    if db.is_album_admin(album_id, user_id):
        raise HTTPException(status_code=400, detail="Album admins cannot leave their own album. Please transfer admin rights first or delete the album.")
...
def update_album_privacy(album_id: int, request: PrivacyModeRequest, user_id: int = Depends(get_current_user)):
    ...
    if request.privacy_mode not in ['public', 'private']:
        raise HTTPException(status_code=400, detail="Privacy mode must be 'public' or 'private'")
...
def create_album_invite(album_id: int, request: CreateInviteRequest, user_id: int = Depends(get_current_user)):
    ...
    invite = db.create_album_invite(album_id, user_id, request.expires_days, request.max_uses)
...
def join_album_via_invite(invite_token: str, user_id: int = Depends(get_current_user)):
    ...
    if not success:
        raise HTTPException(status_code=400, detail="Invite link is expired or no longer valid")
    
    # Get album info
    albums = db.get_user_albums(user_id)
    album = next((a for a in albums if a["album_id"] == invite["album_id"]), None)
...
def get_invite_info(invite_token: str):
    ...
    conn = sqlite3.connect(db.db_path)
    cursor = conn.cursor()
    cursor.execute("SELECT name, admin_user_id FROM albums WHERE album_id = ?", (invite["album_id"],))
    row = cursor.fetchone()
    conn.close()
...
def get_people(album_id: Optional[int] = None, user_id: int = Depends(get_current_user)):
    ...
    if album_id:
        ...
        persons = db.get_all_persons(album_id, user_id, privacy_mode)
    else:
        ...
        for aid in album_ids:
            album = next((a for a in user_albums if a["album_id"] == aid), None)
            if album:
                pmode = album.get('privacy_mode', 'public')
                persons_in_album = db.get_all_persons(aid, user_id, pmode)
...
def auto_merge_people(threshold: float = 0.5):
    ...
    try:
        ...
        merge_pairs = clusterer.find_mergeable_clusters(person_embeddings, threshold)
...
def fix_data_integrity():
    ...
    try:
        ...
        cursor.execute("SELECT face_id, embedding FROM faces WHERE person_id IS NULL AND embedding IS NOT NULL")
...
async def search_face(file: UploadFile = File(...)):
    ...
    try:
        ...
        if results:
            ...
            print(f"Distance range: min={min(distances):.3f}, max={max(distances):.3f}, avg={sum(distances)/len(distances):.3f}")
        ...
        for face_id, distance in results:
            cursor.execute("""
                SELECT f.photo_id, ph.file_path, f.person_id, p.name
                FROM faces f
                JOIN photos ph ON f.photo_id = ph.photo_id
            ...
            if row and row[0] not in seen_photos:
                photo_id, file_path, person_id, person_name = row
...
def migrate_to_albums(user_id: int = Depends(get_current_user)):
    ...
    try:
        ...
        cursor.execute("UPDATE photos SET album_id = ? WHERE album_id IS NULL", (default_album_id,))
        updated_photos = cursor.rowcount
        
        # Update all persons without album_id
        cursor.execute("UPDATE persons SET album_id = ? WHERE album_id IS NULL", (default_album_id,))
...
def assign_admin_to_albums(user_id: int = Depends(get_current_user)):
    ...
    try:
        ...
        cursor.execute("UPDATE albums SET admin_user_id = ? WHERE admin_user_id IS NULL", (user_id,))
